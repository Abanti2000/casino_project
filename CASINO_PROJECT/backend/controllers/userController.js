...
//       [dbUser.phone]
//     );
//     const totalRecharge = recharge.reduce((sum, r) => sum + r.money, 0);

//     const [withdraw] = await connection.query(
//       "SELECT * FROM withdraw WHERE phone = ? AND status = 1",
//       [dbUser.phone]
//     );
//     const totalWithdraw = withdraw.reduce((sum, w) => sum + w.money, 0);

//     const { password, token, ...safeUser } = dbUser;
  ...
  if (data) {
    ...
    if (data == 2) {
      ...
      if (check >= get && point_list.total2 != 0) {
        ...
        return res.status(200).json({
          message: "Please Recharge ₹ 8001 to claim gift.",
          status: false,
          timeStamp: timeNow,
        });
